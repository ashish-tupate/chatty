	...
	public static ArrayList<HashMap<String, Object>> getUserLastMessagesByGroup(Integer userId, Integer groupId)
	{
		...
		.append("FROM ( ")
			...
			.append("WHERE gu.GROUP_ID = ? ")
				...
				.append(") ")
				.append("AND ( ")
					.append("CASE WHEN gu.STATUS = 0 THEN ")
						.append("( CASE WHEN TO_NUMBER( TO_CHAR(m.INSERT_AT,'yyyymmddhh24missff')) < TO_NUMBER( TO_CHAR(gu.UPDATE_AT,'yyyymmddhh24missff')) THEN 1 ELSE 0 END ) ")
					.append("ELSE 1 END ")
				.append(") = 1 ")
				.append("ORDER BY m.MESSAGE_ID DESC ")
				.append("OFFSET 0 ROWS FETCH NEXT 5 ROWS ONLY ")
		.append(") ms ")
		.append("ORDER BY ms.MESSAGE_ID ASC");
		...
		try {
			...
			while(resultSet.next())
			{
				...
				listItem.put("text", resultSet.getString(3));
				listItem.put("owner", resultSet.getString(2));
